function PlayMode(MAX_BG_ORGS,MAX_SCREEN_ORGS,gs) {
	var MAX_SCREEN_TIME = 60 * 3;		// number of frames that each organism stays on screen before going back into the background population
	...
	function playScreen() {
		...
		if (!(frameCount%10))
			health--;
		document.getElementById('score').innerHTML='Health: ' + health;
		if (!(frameCount%10))
			showStats();
		if (health == 0)
